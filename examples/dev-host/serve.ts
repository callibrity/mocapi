#!/usr/bin/env npx tsx
/**
 * HTTP servers for the mocapi MCP Apps dev host:
 * - Host server (port 5173): serves the host React SPA (index.html)
 * - Sandbox server (port 5174): serves sandbox.html with CSP headers
 *
 ...
 *
 * Adapted from https://github.com/modelcontextprotocol/ext-apps
 * examples/basic-host/serve.ts — CSP logic (sanitizeCspDomains /
 * buildCspHeader) kept verbatim.
 */
...
const HOST_PORT = parseInt(process.env.HOST_PORT || "5173", 10);
const SANDBOX_PORT = parseInt(process.env.SANDBOX_PORT || "5174", 10);
...
// API endpoint to get the sandbox origin so the host can validate/target
// postMessage traffic to the correct cross-origin sandbox proxy.
hostApp.get("/api/sandbox-origin", (_req, res) => {
  res.json({ origin: `http://localhost:${SANDBOX_PORT}` });
});
