...
- **`@McpUi(resource=…)` serve-mode.** Serve a `ui://` bundle from a fixed classpath/filesystem location with no resource method; mocapi contributes a public resource resolved once at startup and fails the boot on a missing bundle or the same URI served from two locations ([ADR-0036](docs/adr/0036-mcpui-serve-mode.md)).
- **Function-backed resource readers and a `ResourceContributor` SPI.** Resource handlers are built around a reader function, and `McpResourcesService` merges every `ResourceContributor` bean at construction (the `@McpResource` annotation scan is the primary contributor). A `@McpResource` method may now return `String`/`CharSequence`, `byte[]`/`ByteBuffer`, or a Spring `Resource` (text vs. blob via a new `content` enum), in addition to `ReadResourceResult` ([ADR-0035](docs/adr/0035-resource-readers-and-contributor-seam.md)).
