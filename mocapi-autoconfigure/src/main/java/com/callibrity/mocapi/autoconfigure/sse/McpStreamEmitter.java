...
/**
 * Wrapper around Spring's {@link SseEmitter} that provides enhanced logging, error handling, and
 * event ID management for MCP SSE streams.
 *
 * <p>This class is based on the fault-tolerant SSE pattern from calli-api's ChatCompletionEmitter,
 * adapted for MCP protocol streaming requirements.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Thread-safe completion handling with idempotency via {@link AtomicBoolean}
 *   <li>Automatic error recovery and cleanup on client disconnects or network failures
 *   <li>Event ID tracking for stream resumability per MCP 2025-11-25 spec
 *   <li>Detailed logging for debugging SSE stream lifecycle
 * </ul>
 *
 * @see SseEmitter
 * @see McpSession
...
public class McpStreamEmitter {
  ...
  /**
   * Completes the emitter with an error. This method is idempotent - multiple calls will only
   * complete the emitter once.
   *
  ...
  private void completeWithError(Throwable ex) {
    if (!completed.compareAndSet(false, true)) {
      log.trace(
          "[MCP-SSE:{}:{}] completeWithError called but stream already closed",
          sessionId,
          streamId);
      return;
    }
  ...
  /**
   * Completes the SSE stream normally. This method is idempotent - multiple calls will only
   * complete the emitter once.
   */
  public void complete() {
    if (!completed.compareAndSet(false, true)) {
      log.trace("[MCP-SSE:{}:{}] complete() called but stream already closed", sessionId, streamId);
      return;
    }
  ...
  /**
   * Sends a priming event with an empty data field to establish the connection and enable client
   * reconnection via Last-Event-ID.
   *
   * <p>Per MCP 2025-11-25 spec: "Server SHOULD immediately send SSE event with event ID and empty
  ...
  /**
   * Sends data as an SSE event with automatic event ID generation. The event is stored in the
   * session for potential redelivery.
   *
  ...
  /**
   * Sends a final event and then completes the stream normally. If the send fails, the stream will
   * be completed with an error instead.
   *
   ...
   * @return {@code true} if the send was successful, {@code false} if it failed or the stream was
   *     already completed
   */
  private boolean trySendInternal(SseEvent event) {
    if (completed.get()) {
      log.trace(
          "[MCP-SSE:{}:{}] Attempt to send event {} after completion ignored",
          sessionId,
          streamId,
          event.id());
      return false;
    ...
    try {
      ...
      log.debug(
          "[MCP-SSE:{}:{}] Client disconnected while sending event {}: {}",
          sessionId,
          streamId,
          event.id(),
          e.toString());
      ...
      log.error(
          "[MCP-SSE:{}:{}] Unexpected exception while sending event {}",
          sessionId,
          streamId,
          event.id(),
          e);
