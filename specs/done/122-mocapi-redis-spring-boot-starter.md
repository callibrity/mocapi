# mocapi-redis-spring-boot-starter (all-in-one Redis starter)

## What to build

Create a new Maven module `mocapi-redis-spring-boot-starter`
that is a pure aggregation pom. It declares dependencies on every
...
Spring Boot's documented naming convention for third-party
starters is `<name>-spring-boot-starter` (not
`spring-boot-starter-<name>`, which is reserved for Spring
Boot's own starters). So:

- Existing: `mocapi-spring-boot-starter` (the base starter — name
  is `mocapi`, suffix is `-spring-boot-starter`)
- **This spec**: `mocapi-redis-spring-boot-starter` (name is
  `mocapi-redis`, suffix is `-spring-boot-starter`)
...
substrate journal, substrate notifier), each with its own
auto-configuration. That's error-prone — miss one and you
silently fall back to in-memory for that capability. The
all-in-one starter makes "run mocapi on Redis" a single
coordinate.
...
<project ...>
  ...
  <name>Mocapi - Redis Spring Boot Starter</name>
  <description>
...
to add it themselves — pulling in the starter gives them
everything. The individual mocapi modules keep the Redis starter
as `optional` to avoid forcing it on non-Redis users; the starter
is the place where we commit to Redis.
...
- [ ] New module `mocapi-redis-spring-boot-starter` exists as a
...
- [ ] A downstream integration test (in a new dedicated test
      module or inside `mocapi-session-store-redis`) uses
      `@SpringBootTest` with the starter on the classpath and
      Testcontainers Redis, verifies that all four beans
      (`McpSessionStore`, `Mailbox`, `Journal`, `Notifier`) are
      the Redis-backed implementations and not the in-memory
      fallbacks.
...
- **Spring Boot naming convention** — this starter must be
  named `mocapi-redis-spring-boot-starter`, NOT
  `mocapi-spring-boot-starter-redis`. The `<name>-spring-boot-starter`
  form is the documented third-party convention; the
  `spring-boot-starter-<name>` form is reserved for Spring Boot's
  own starters.
- **Dependency on spec 113 and any substrate prerequisites**.
  ...
  as separate modules, this spec is blocked on them too — verify
  before starting.
...
- **Commit suggestion**: one commit for the new module + parent
  pom update + any integration test.
