...
- Depends on spec 182 (context propagation across the HTTP
  transport's VT spawn). Without 182, observations started inside
  the handler VT have no parent HTTP span — meters still work, but
  tracing dashboards show orphan handler spans.
- Follow-up spec — `mocapi-actuator-spring-boot-starter`
