...
for i in $(seq 1 8); do docs/guides/performance/soak.sh 45 1 $i & done
wait
...
rm -f /tmp/soak-*.log /tmp/soak-count-*.txt
date "+start %H:%M:%S"
for i in $(seq 1 16); do docs/guides/performance/soak.sh 180 1 $i & done
wait
date "+end %H:%M:%S"
...
Capture your numbers in `docs/guides/performance/history.md` when you run. The
...
- JFR files live in `/tmp`; archive interesting ones under
  `docs/guides/performance/jfr/<date>-<label>.jfr` before they get blown away.
