...
The framework discovers every bean in the context, scans its methods for `@McpTool`, and registers one handler per annotated method. Each registered tool is logged at `INFO` level during startup (see [Startup Logging](../design/architecture-overview.md#startup-logging) for the full catalog).
