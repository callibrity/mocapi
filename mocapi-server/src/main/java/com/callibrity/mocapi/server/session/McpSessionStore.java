...
public interface McpSessionStore {
  ...
  /** Saves a session using its {@link McpSession#sessionId()} as the key, with a TTL. */
  void save(McpSession session, Duration ttl);

  /** Replaces session data for an existing session, preserving the current TTL. */
  void update(String sessionId, McpSession session);

  /** Finds a session by ID; returns empty if not found or expired. */
