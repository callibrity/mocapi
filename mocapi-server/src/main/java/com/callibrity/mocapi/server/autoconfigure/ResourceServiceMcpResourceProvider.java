...
import com.callibrity.mocapi.server.resources.annotation.ResourceServiceScanner;
    ...
    beans.forEach(
        (beanName, bean) -> {
          log.info(
              ...
              beanName);
          var rs = ResourceServiceScanner.createResources(invokerFactory, bean);
          ...
          var ts =
              ResourceServiceScanner.createResourceTemplates(
                  invokerFactory, templateResolvers, bean);
          ts.forEach(
              t ->
                  log.info(
