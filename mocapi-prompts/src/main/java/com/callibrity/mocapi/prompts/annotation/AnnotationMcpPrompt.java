...
public class AnnotationMcpPrompt implements McpPrompt {
    ...
    this.promptArguments =
        Arrays.stream(method.getParameters())
            .map(
                p ->
                    new PromptArgument(
                        p.getName(), Parameters.descriptionOf(p), Parameters.isRequired(p)))
            .toList();
  }
  ...
  private static void verifyMethodSignature(Method method) {
    if (!GetPromptResult.class.equals(method.getReturnType())) {
      throw new IllegalArgumentException(
          String.format(
              "Prompt method '%s' returns %s (GetPromptResult is required).",
              method.getName(), ClassUtils.getSimpleName(method.getReturnType())));
    }
    if (!Arrays.stream(method.getParameterTypes()).allMatch(String.class::equals)) {
      throw new IllegalArgumentException(
          String.format("Prompt method '%s' has non-String parameters.", method.getName()));
    }
  }
  ...
  private static Object extractArgumentValue(
      PromptArgument argument, Map<String, String> arguments) {
    var value = arguments.get(argument.name());
    if (value == null && argument.required()) {
      throw new McpInvalidParamsException(
          String.format("Required argument '%s' is missing.", argument.name()));
    }
    return value;
  }
  ...
  public GetPromptResult getPrompt(Map<String, String> arguments) {
    var params = promptArguments.stream().map(p -> extractArgumentValue(p, arguments)).toArray();
    try {
      ...
      throw new McpInternalErrorException(
          String.format("Prompt \"%s\" did not return a GetPromptResult.", name));
