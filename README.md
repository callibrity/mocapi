...
- **`mocapi-o11y`** — metrics + distributed tracing via Micrometer's `Observation` API. One interceptor per handler emits both meters (`mcp.tool` / `mcp.prompt` / `mcp.resource` / `mcp.resource_template`) and OTel spans. Activates when an `ObservationRegistry` bean is present — bring your own meter registry and/or OTel SDK. See [Observability](docs/observability.md).
