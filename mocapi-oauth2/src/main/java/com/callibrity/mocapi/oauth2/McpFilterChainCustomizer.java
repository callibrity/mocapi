...
/**
 * Strategy for layering additional configuration onto the mocapi-managed security chain for the MCP
 * endpoint ({@code /mcp/**}) after it has been configured with OAuth2 resource-server + bearer-
 * token defaults. Use this to require specific scopes, add CORS rules, restrict to certain roles,
 * etc., without having to redeclare the whole {@link
 * org.springframework.security.web.SecurityFilterChain SecurityFilterChain}.
 *
 * <p>Scoped to the MCP endpoint only. The {@code /.well-known/oauth-protected-resource} metadata
 * endpoint is a separate chain with its own customizer SPI ({@link
 ...
 */
@FunctionalInterface
public interface McpFilterChainCustomizer {
